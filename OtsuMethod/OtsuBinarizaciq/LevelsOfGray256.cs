...
{  
    class LevelsOfGray256
    {      
        static int NGRAY = 256;
        ...
        public Bitmap ConvertingToGrayScale(Bitmap bmp)
        {
            ...
            unsafe
            {
                ...
                {
                    p[0] = (byte)((0.299 * p[2]) + (0.587 * p[1]) + (0.114 * p[0])); // with this values we receive good gray colors
                    p[1] = p[0];
                    p[2] = p[0];
        ...
        public void Binarization(Bitmap bmp, int[] t)
        {
            ...
            unsafe
            {
                ...
                for (int i = 0; i < h; i++)
                {
                    ...
                    {
                        row[j] = (byte)((row[j] > (byte)t[1]) ? 255 : 0); // when the color number is greater of the threshold , we set the color to 255 ( white )
                        row[j + 1] = (byte)((row[j + 1] > (byte)t[1]) ? 255 : 0);
                        row[j + 2] = (byte)((row[j + 2] > (byte)t[1]) ? 255 : 0);
                    }
                }
            }
            bmp.UnlockBits(bmData);
